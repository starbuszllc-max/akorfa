            ...
            {selectedConversation ? (
              <>
                ...
                <div 
                  ...
                  style={{
                    backgroundImage: `linear-gradient(rgba(0,0,0,0.95), rgba(0,0,0,0.95)), url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23C9A962' fill-opacity='0.05'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")`
                  }}
                >
                  {messages.length === 0 && selectedConversation.id === 'new' && (
                    <div className="flex items-center justify-center h-full">
                      <div className="text-center bg-[#1a1a1a]/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-[#C9A962]/20">
                        ...
                        <p className="font-semibold text-white">{selectedConversation.otherUser.username}</p>
                            ...
                            {msg.messageType === 'audio' && msg.mediaUrl ? (
                              <div className="flex items-center gap-3 min-w-[200px]">
                                ...
                                <div className="flex-1">
                                  ...
                                  {msg.audioDuration && (
                                    <span className={`text-xs ${isMe ? 'text-black/60' : 'text-gray-400'}`}>{formatTime(msg.audioDuration)}</span>
                                  )}
                                </div>
                              </div>
                              ...
                              <p className={`text-[15px] leading-relaxed ${isMe ? 'text-black' : 'text-white'}`}>
                                {msg.content}
                              </p>
                            )}
                            <div className={`flex items-center gap-1 mt-1 ${isMe ? 'justify-end' : 'justify-start'}`}>
                              <span className={`text-[11px] ${isMe ? 'text-black/60' : 'text-gray-500'}`}>
                                {formatMessageTime(msg.createdAt)}
                              </span>
                              {isMe && (
                                msg.isRead ? (
                                  <CheckCheck className="w-4 h-4 text-black/70" />
                                ) : (
                ...
                <form onSubmit={sendMessage} className="p-3 bg-[#000000] border-t border-[#C9A962]/20">
                  ...
                  <div className="flex items-center gap-2">
                    {isRecording ? (
                      <>
                        <div className="flex-1 flex items-center gap-3 px-4 py-3 bg-red-900/30 rounded-full border border-red-500/30">
